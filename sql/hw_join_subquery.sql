...
select count(id) as '행수' from user full join board;
...
select user.id , count(createtime) as '개수' from user inner join board on user.name = board.writername where year(board.createtime) > '2016' group by user.id;
...
select id from user where name in (select writername from board where year(createtime) = '2019');
...
select name from user where user.name in (select writername from board where board.writername = user.name and user.age > 60);